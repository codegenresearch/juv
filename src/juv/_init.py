...
from ._pep723 import Pep723Meta
...
def new_notebook_with_inline_metadata(dir: Path, python: str | None = None) -> dict:
    ...
    with tempfile.NamedTemporaryFile(
        ...
        meta = Pep723Meta.from_toml(contents) if Pep723Meta.from_toml(contents) else None
        notebook = new_notebook(cells=[code_cell(contents, hidden=True)])

        if meta:
            if meta.requires_python and not python:
                python = meta.requires_python

            if meta.dependencies:
                cmd.extend(["--with", ",".join(meta.dependencies)])

    return notebook, python
...
def init(
    ...
    notebook, python_version = new_notebook_with_inline_metadata(path.parent, python)
    write_ipynb(notebook, path)

    rich.print(f"Initialized notebook at `[cyan]{path.resolve().absolute()}[/cyan]`")
    if python_version:
        rich.print(f"Using Python version: `[cyan]{python_version}[/cyan]`")